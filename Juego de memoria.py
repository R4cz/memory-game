...
# Colores
color_fondo = (39,39,42)
color_green = (82, 183, 136)
...
anchura_boton = anchura_pantalla/5
...
yFuente = int(altura_pantalla - altura_boton + 2)

# El botón, que al final es un rectángulo
boton_iniciar = pygame.Rect(anchura_pantalla/5, altura_pantalla - altura_boton, anchura_pantalla/5, altura_boton)
boton_ranked = pygame.Rect((anchura_pantalla/5) * 3, altura_pantalla - altura_boton, anchura_pantalla/5, altura_boton)
...
def comprobar_si_gana():
    if gana():
        ...
        registro_jugador = RegisterWindow()
        nombre, edad = registro_jugador.get_data()
        ...
        global tiempo_total
        tiempo = fin_juego - inicio_juego
        tiempo_total = time_converter(int(tiempo))

        añadir_jugador_ranking()
...
# Funciones para crear la ventana del ranking
def añadir_jugador_ranking():
    data = ( nombre_jugador, edad_jugador, tiempo_total, numero_errores)
    connection.add_to_ranking(data)
...
def show_ranking():
    players_ranking = connection.recover_ranking()
...
while True:
    ...
    for event in pygame.event.get():
        ...
        # Si hicieron clic y el usuario puede jugar...
            ...
            if boton_ranked.collidepoint(event.pos) or boton_iniciar.collidepoint(event.pos):

                if boton_ranked.collidepoint(event.pos):
                    if not juego_iniciado:
                        show_ranking()

                elif boton_iniciar.collidepoint(event.pos):
                    if not juego_iniciado:
                        iniciar_juego()
                ...
                if x1 is None and y1 is None:
                    ...
                    if cuadro1.fuente_imagen == cuadro2.fuente_imagen:
                        ...
                        puede_jugar = False
                        # Contador de errores, cuenta la cantidad de veces que no hicimos par.
    ...
    pantalla_juego.fill(color_fondo)
    ...
    if juego_iniciado:
        ...
        pygame.draw.rect(pantalla_juego, color_ocultar_botones, boton_iniciar)
        pantalla_juego.blit(fuente.render(
            "Iniciar juego", True, color_fondo), (anchura_pantalla/5 + anchura_boton/6, yFuente))

        pygame.draw.rect(pantalla_juego, color_ocultar_botones, boton_ranked)
        pantalla_juego.blit(fuente.render(
            "Ranking", True, color_fondo), ((anchura_pantalla/5) *3 + anchura_boton/4, yFuente))

    else:
        pygame.draw.rect(pantalla_juego, color_green, boton_iniciar)
        pantalla_juego.blit(fuente.render(
            "Iniciar juego", True, color_fondo), (anchura_pantalla/5 + anchura_boton/6, yFuente))

        pygame.draw.rect(pantalla_juego, color_green, boton_ranked)
        pantalla_juego.blit(fuente.render(
            "Ranking", True, color_fondo), ((anchura_pantalla/5) *3 + anchura_boton/4, yFuente))
